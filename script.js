...
function getRequestConfig() {
  const configuredValue = typeof apiKey === 'string' ? apiKey.trim() : '';

  if (
    configuredValue === '' ||
    configuredValue === 'PASTE_YOUR_OPENAI_API_KEY_HERE'
  ) {
    return {
      error: 'Request failed: Missing apiKey value in secret.js.'
    };
  }

  // If apiKey looks like a URL, treat it as a proxy endpoint.
  ...
  return {
    ...
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${configuredValue}`
    }
  };
}
